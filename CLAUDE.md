...
**JUnit `@Test` classes** — `plain/P1GoogleTest`, `plain/P2AllModelsTest`, `LC1GoogleUserTextTest`, `LC2AllModelsUserTextTest`, `LC3UserMessageTest`, `LC4SystemAndUserMessagesTest`, `LC10EmbeddingTest`. All but the last run under a bare `mvn test`; `LC10EmbeddingTest` is skipped by Surefire's name filter (see Build & run).
