...
- `mvn compile` — build.
- `mvn test` — runs `P1GoogleTest`, `P2AllModelsRefactoredTest`, `LC1GoogleUserTextTest`, `LC2AllModelsUserTextTest`, `LC3UserMessageTest`, `LC4SystemAndUserMessagesTest`, and `LC10EmbeddingTest`. The remaining `LC*` classes are interactive `main()` methods, not tests, so Surefire never picks them up. Surefire's default includes are `Test*` / `*Test` / `*Tests` / `*TestCase` and the pom does not override them, so a new experiment only joins the default run if its class name ends in `Test` — which is why `LC10EmbeddingTest` carries the suffix while the interactive classes deliberately do not. Note that a bare `mvn test` therefore calls all four providers plus an OpenAI embedding, and every one of those is billed.
...
**JUnit `@Test` classes** — `plain/P1GoogleTest`, `plain/P2AllModelsRefactoredTest`, `LC1GoogleUserTextTest`, `LC2AllModelsUserTextTest`, `LC3UserMessageTest`, `LC4SystemAndUserMessagesTest`, `LC10EmbeddingTest`. All of them run under a bare `mvn test`, since every name ends in `Test` (see Build & run).
